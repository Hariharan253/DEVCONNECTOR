...
router.get('/user/:user_id',async (req, res) => {
    try {
        const profiles = await Profile.find({user: req.params.user_id }).populate('user',['name','avatar']);

        if(!profiles) return res.status(500).json({msg:'Profile Not Found'});
        res.json(profiles);
    } catch (err) {
        console.error(err.message);
        if(err.kind == 'user_id') return res.status(500).json({msg: 'Profile Not Found'});
        res.status(500).send('server error');
    }
});
...
router.delete('/', auth, async (req, res) => {
    ...
    res.json({msg: 'User and profile of that user is deleted'});    
    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error');
    }
